...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!coordinates) {
      throw new Error('Missing coordinates. Please draw a parcel polygon before running analysis.');
    }

    if (!Array.isArray(coordinates) || !coordinates[0] || coordinates[0].length < 3) {
...
function validateStructuredAnalysis(analysis: any, context: any): any {
  ...
  if (isGeneric(analysis.decision) || isGeneric(analysis.key_strength) || isGeneric(analysis.primary_risk)) {
    ...
    return buildDataDrivenSummary(context, context.roi, context.foundation, context.geological, context.zoning);
  }
...
function calculateFoundationCosts(geological: any, zoning: any, siteArea: number) {
  ...
  const foundationType =
    soilScore >= 85 && gwDepth > 10 ? 'shallow-spread' : soilScore >= 70 ? 'reinforced-mat' : 'deep-pile';
...
function calculateROI(geological: any, zoning: any, foundation: any, siteArea: number) {
  ...
  const geoRisk = (geological.riskLevel as string) === 'LOW' ? 0.95 : geological.riskLevel === 'MODERATE' ? 0.85 : 0.7;
...
function calculateConfidenceScore(geological: any, foundationScore: number, roi: number, risk: number): number {
  ...
  return Math.round(geoScore * 0.25 + foundationScore * 0.25 + roiScore * 0.3 + risk * 100 * 0.2);
}
